...
expect_equal(sb_rp(x, lvl = c("Female", "Male"), rp = c("F", "M")), x2)

x2 <- x
x2[1:2] <-  c("Female", "Male")
expect_equal(sb_rp(x, i = 1:2, rp = c("Female", "Male")), x2)
expect_equal(sb_rp(x, i = c(TRUE, TRUE, rep(FALSE, 14)), rp = c("Female", "Male")), x2)

x2 <- x
x2[which(names(x2) == "a")] <- "Male"
expect_equal(sb_rp(x, i = "a", rp = "Male"), x2)
  ...
  for(i in 1:length(row)) {
    for(j in 1:length(col)) {
      if(!rlang::is_missing(row[[i]]) && !rlang::is_missing(col[[j]])) {
        ...
        rp <- seq_len(len)
        expect_equal(
          sb_rp(x, row = row[[i]], col = col[[j]], rp = rp),
...
expect_error(
  sb_rp(x, i = 1),
  pattern = "argument \"rp\" is missing, with no default"
)
expect_error(
  sb_rp(x, i = 1:5, rp = 1:6),
...
expect_error(
  sb_rp(x, i = 1:5, rp = 1:10),
  pattern = "`rp` must be a list"
)
expect_error(
  sb_rp(x, i = 1:5, rp = as.list(1:6)),
  pattern = "recycling not allowed"
)
enumerate <- enumerate + 3
...
expect_error(
  sb_rp(x, row = 1:2, col = 1:2, rp = 1:6),
  pattern = "recycling not allowed"
)
...
expect_error(
  sb_rp(x, list(1:2, 1:2), c(1,3), rp = 1:6),
  pattern = "recycling not allowed"
)
expect_error(
  sb_rp(x, list(1:2, 1:2), c(2,3), rp = 1:6),
  pattern = "recycling not allowed"
)
