  ...
  for(i in 1:length(row)) {
    for(j in 1:length(col)) {
      # if(!rlang::is_missing(row[[i]]) && !rlang::is_missing(col[[j]])) {
        expect_equal(
          sb_tf(x, row = row[[i]], col = col[[j]], tf = mean),
          subset_mat(x, row[[i]], col[[j]])
        ) |> errorfun()
        expect_true(sb_tf(x, row = row[[i]], col = col[[j]], tf = mean) |>
                      is.matrix()) |> errorfun()
      ...
      for(k in 1:length(filter)) {
        for(l in 1:length(get_vars)) {
          ...
          if(!wrong1 && !wrong2 && !wrong3) {
            ...
            expect_equivalent(
              sb_tf(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]], tf = \(x)x[1]),
              mysubset(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]])
            ) |> errorfun()
            expect_true(
              sb_tf(x, row[[i]], col[[j]], filter[[k]], get_vars[[l]], tf = \(x)x[1]) |> is.data.frame()
            )
            assign("enumerate", enumerate + 2, envir = parent.frame(n = 1))
          }
        }
...
# test errors ====
sb_test <- function(...)sb_tf(..., tf = \(x)x[1])
sys.source(file.path(getwd(), "source", "sourcetest-errors.R"), envir = environment())

expect_error(
  sb_tf(1:10, i = 1),
  pattern = "argument \"tf\" is missing, with no default"
)
