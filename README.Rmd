...
The Goal of the 'subsets' package is NOT to replace the square-brackets operators (`[`, `[[`, `[<-`, and `[[<-`), but to provide **alternative** sub-setting methods and functions, to be used in situations where the square-brackets operators are inconvenient. 
...
These are (hopefully) easier sub-setting methods and functions with the following properties:
  ...
  * *Explicit copy semantics*:
      ...
      * For sub-set operations that just change values in-place
        ...
        a method that modify the object by *reference*, or choose
        a method that return a *deep copy*.
...
sequences and indices (sometimes needed in sub-setting) are provided:
