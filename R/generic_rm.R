...
#' @rdname sb_rm
#' @export
sb_rm.default <- function(x, i, ..., rat = FALSE) {
...
sb_rm.matrix <- function(
    x, row = NULL, col = NULL, i = NULL, ..., rat = FALSE
) {
...
sb_rm.array <- function(
    x, idx = NULL, dims = NULL, rcl = NULL, i = NULL, ..., rat = FALSE
) {
  ...
  if(!is.null(rcl)) {
    ...
    return(.sb3d_rm(x, rcl[[1]], rcl[[2]], rcl[[3]], rat = rat))
  }
  
  if(rat) {
    x <- .fix_attr(.arr_rm(x, idx, dims, abortcall = sys.call()), attributes(x))
  } else{ x <- .arr_rm(x, idx, dims, abortcall = sys.call()) }
  return(x)
}
...
#' @rdname sb_rm
#' @export
sb_rm.factor <- function(x, i = NULL, lvl = NULL, drop = FALSE, ..., rat = FALSE) {
...
#' @rdname sb_rm
#' @export
sb_rm.list <- function(x, i, drop = FALSE, ..., rat = FALSE) {
