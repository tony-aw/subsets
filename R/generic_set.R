...
#' 
#'
#' @param x a vector, vector-like object, data.frame, data.frame-like object, or a list.
...
sb_set.matrix <- function(x, row = NULL, col = NULL, i = NULL, ..., rp, tf) {
  ...
  if(is.null(i)) {
    ...
    elements <- coord2ind(coords, dim(x), length(x), checks = FALSE)
