...
#' @param coe Unlike the other object types,
#' data.frame-like objects do NOT automatically coerce types. \cr
#' Therefore, the user can specify a coercion function,
#' to be applied on the entirety of every column of the subset.
#' Columns outside the subset are not affected. \cr
#' This coercion function is applied BEFORE replacement (`rp`) or transformation (`tf()`). \cr
#' By default, `coe = NULL` which means no entire columns are transformed. \cr
...
#' # above is equivalent to obj[1:3, , 1:2] <- -1:-12
#' sb_mod(obj, i = \(x)x<=5, rp = -1:-5)
