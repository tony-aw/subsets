...
#' where each single character in a string is a single element. \cr \cr
#' 
...
sb_str <- function(str, ind, rp.str, rp.ind) {
  if(missing(rp.str) && missing(rp.ind)) {
    ...
    return(.rcpp_sb_str(str, as.integer(ind - 1)))
  } else {
    if(length(ind) != length(rp.ind)) {
      stop("`ind` and `rp.ind` must be of same size")
    }
    if(.rcpp_str_ind_is_nonpos(ind, rp.ind)) {
      stop("`ind` and `rp.ind` must be strictly positive integers")
    }
    return(.rcpp_sb_str_rp(str, as.integer(ind - 1), rp.str, as.integer(rp.ind - 1)))
