...
#' IMPORTANT: The length of `inits` determines the window size. \cr
#' For a regular Fibonacci, \code{inits = 0:1},
#' which of course means a window size of 2. \cr
#' @param n a single integer,
#' giving the size of the numeric vector to generate. \cr
#' NOTE: it must hold that `n` is larger than or equal to the window size.
#' The window size is equal to `length(inits)`.
